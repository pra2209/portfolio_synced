...
var githubProjects = mysqlTable("github_projects", {
  id: varchar("id", { length: 100 }).primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description").notNull(),
  tags: text("tags"),
  // JSON array of tech tags
  url: varchar("url", { length: 500 }).notNull(),
  featured: boolean("featured").default(false),
  order: int("order").default(0),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
...
async function getWritingPosts(category) {
  const db = await getDb();
  if (!db) return [];
  try {
    if (category && category !== "all") {
      return await db.select().from(writingPosts).where(eq(writingPosts.category, category)).orderBy(desc(writingPosts.publishedAt));
    }
...
async function getProjects(featured) {
  const db = await getDb();
  if (!db) return [];
  try {
    if (featured) {
...
async function getGitHubProjects(featured) {
  const db = await getDb();
  if (!db) return [];
  try {
    if (featured) {
